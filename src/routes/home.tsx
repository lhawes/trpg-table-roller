...
const api = 'https://v2bgn4ta39.execute-api.us-east-1.amazonaws.com/prod/';
...
export default function Home() {
  ...
  const [fetchcheck, setFetchcheck] = useState();

  const fetchCognito = async () => {
    console.log('start api call')
    
    const response = await fetch( api, { method: 'get', headers: new Headers({ 'Authorization': auth.sessionInfo?.idToken || '' }) })
  ...
  return (
    <Grid container>
      <Grid className={classes.root} container direction="column" justify="center" alignItems="center">
        ...
        <Box m={2}>
          <button onClick={() => fetchCognito()}>Hit up cognito</button>
          fetchcheck
          <pre className={classes.session}>{JSON.stringify(fetchcheck, null, 2)}</pre>
        </Box>
        <Box m={2}>
